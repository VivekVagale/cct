...
export function ProjectOptionCard({
  ...
  return (
    /*
     * Three boxes, same as VehicleCard and for the same two reasons.
     *
     * The outer one measures and listens: `useTilt` reads
     * `getBoundingClientRect()` on every pointer move, and a rotated box reports
     * a larger rect — which moves the pointer fraction, which changes the
     * rotation. The ref cannot live on anything that turns.
     *
     * The middle one rotates and carries the beam with it. Wrapped outside the
     * rotation, the border stayed flat while the card tilted away from it, which
     * showed worst on the selected card because that is the one lit.
     */
    <motion.div
      ref={ref}
