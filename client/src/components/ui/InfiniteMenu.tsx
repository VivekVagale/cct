/**
 * InfiniteMenu — vendored from React Bits (TypeScript + CSS variant).
 *
 * A WebGL2 sphere of image discs you can drag; it snaps to the nearest disc
 * and surfaces that item's title, description and link. Kept close to the
 * upstream source so it stays easy to diff against a future version; the
 * accompanying CSS is the part adapted to Cold Chain Theory's palette.
 */
...
class InfiniteGridMenu {
  ...
  /** Stops the render loop. Without this the requestAnimationFrame chain
   *  outlives the component and keeps a dead canvas animating forever. */
  public dispose(): void {
    this.disposed = true;
  }
      ...
      this.items.map(
        item =>
          new Promise<HTMLImageElement | null>(resolve => {
            ...
            // A failed image must not reject the whole atlas, or none of the
            // discs get a texture and the sphere renders blank.
