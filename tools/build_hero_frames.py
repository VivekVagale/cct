...
def main():
    ...
    ap.add_argument("--crop-aspect", type=float, default=None,
                    help="crop to this width/height ratio, centred, before "
                         "resizing. Used to cut the portrait sequence phones "
                         "get: they see about a quarter of the landscape "
                         "frame's width and upscale it anyway, so shipping the "
                         "full width is paying to send pixels that are thrown "
                         "away at the crop.")
    ap.add_argument("--url-base", default="/hero")
    ap.add_argument("--export", dest="export_name", default="HERO_SEQUENCE")
    args = ap.parse_args()
    ...
    while True:
        ...
        if n in picks:
            ...
            # Crop *after* matting, never before. The matte finds the backdrop
            # by flood-filling black inward from the frame border; on a crop the
            # character reaches the edge, so there is no border of backdrop to
            # start from and the fill either escapes into the body or finds
            # nothing at all.
            if args.crop_aspect:
                cw = int(round(im.height * args.crop_aspect))
                x0 = (im.width - cw) // 2
    ...
    manifest = {
        ...
        "pattern": "f_{i}.webp",
        "sourceFrames": n_src,
        "urlBase": args.url_base,
        "export": args.export_name,
        "cropAspect": args.crop_aspect,
    }
    with open(os.path.join(args.out, "manifest.json"), "w") as f:
        json.dump(manifest, f, indent=2)

    write_sequence_module()

    print(f"wrote {args.count} frames, {total/1e6:.2f} MB total "
          f"({total/args.count/1024:.1f} KB avg), {args.width}x{height}")


# Every sequence directory that has been built. Listed rather than derived so a
# variant cannot be silently dropped by a rebuild of the other one.
SEQUENCE_DIRS = ["client/public/hero", "client/public/hero-m"]


def write_sequence_module():
    """
    Regenerate the typed module from whichever manifests exist.

    Rebuilt from the manifests rather than from this run's arguments, because
    there is more than one sequence now and a run only knows about its own.
    Writing just the variant that was built would delete the other one's export
    and break the Hero on whichever platform was not rebuilt last.
    """
    ts = os.path.join("client", "src", "data", "heroSequence.ts")
    if not os.path.isdir(os.path.dirname(ts)):
        return
    ...
    for d in SEQUENCE_DIRS:
        ...
        with open(path) as f:
            m = json.load(f)
        base = m.get("urlBase", "/hero")
        blocks.append(
            f"export const {m.get('export', 'HERO_SEQUENCE')} = {{\n"
            f"  count: {m['count']},\n"
            f"  width: {m['width']},\n"
            f"  height: {m['height']},\n"
            f"  srcFor: (i: number) => `{base}/f_${{String(i).padStart(4, \"0\")}}.webp`,\n"
            "} as const;\n"
        )

    with open(ts, "w") as f:
        f.write("// Generated by tools/build_hero_frames.py — do not edit by hand.\n")
        f.write("\n".join(blocks))
    print(f"wrote {ts} ({len(blocks)} sequence(s))")
